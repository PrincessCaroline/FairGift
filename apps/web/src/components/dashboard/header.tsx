...
import { GiftIcon, CogIcon, ChevronDownIcon, PlusIcon } from '@heroicons/react/24/solid';
...
export default function Header({ groups, setGroupIdSelected, groupIdSelected }: HeaderProps) {
    const router = useRouter();
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const groupName = groups.find(group => group.id === groupIdSelected)?.name ?? "";
                ...
                <div className="absolute left-0 top-full w-full bg-white border rounded-lg shadow-lg z-10 mt-2">
                    <ul>
                        ...
                        >
                            ...
                            <div>
                                <span className="font-medium text-pink-500">Créer un Groupe</span>
                                <p className="text-sm text-gray-500">Pour votre famille, vos amis, ...</p>
                            </div>
                        </li>
                    </ul>
                </div>
